...
impl MockAdapter {
    fn with_seed_data() -> Self {
        ...
        nodes.insert("/configs/payment/switches".into(), {
        ...
        nodes.insert("/services/gateway".into(), {
        ...
        nodes.insert("/services/session_blob".into(), {
            // Actual binary bytes representing a Java-serialized object (decoded from hex).
            let raw: &[u8] = &[
                0xAC, 0xED, 0x00, 0x05, 0x73, 0x72, 0x00, 0x12, 0x63, 0x6F, 0x6D, 0x2E, 0x65, 0x78,
                0x61, 0x6D, 0x70, 0x6C, 0x65, 0x2E, 0x53, 0x65, 0x73, 0x73, 0x69, 0x6F, 0x6E,
            ];
            let interp = interpret_data(raw);
            NodeDetailsDto {
                path: "/services/session_blob".into(),
